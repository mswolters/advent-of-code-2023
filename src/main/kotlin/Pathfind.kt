...
fun <N> findPath(start: N, isEnd: (node: N) -> Boolean, edgesForNode: (node: N, withPath: Path<N>) -> List<Pair<N, Double>>): Path<N> {
...
fun <N> findPathAStar(start: N, isEnd: (node: N) -> Boolean, edgesForNode: (node: N, withPath: Path<N>) -> List<Pair<N, Double>>, potential: (node: N) -> Double): Path<N> {
    ...
    return findPath(start, isEnd, modifiedEdgesForNode)
}